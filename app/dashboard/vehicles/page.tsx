...
import { getVehicleListings, type Vehicle } from '@/lib/actions/listings'
...
export default async function VehiclesPage() {
  try {
    ...
    // Use the direct server action instead of API fetch
    const { data: vehicles } = await getVehicleListings({ status: 'all' });
    ...
    console.error("Error rendering vehicles page:", error);
    return (
      <div className="container mx-auto p-6">
        <h1 className="text-2xl font-bold mb-4">Something went wrong</h1>
        <p className="text-red-500">There was an error loading the vehicles. Please try again later.</p>
      </div>
    );
  }
} 